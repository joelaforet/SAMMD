...
def test_missing_cosolvent_density_fails_clearly() -> None:
    ...
    with pytest.raises(ValueError, match="requires density_g_ml"):
        plan_solution_components(
            ...
            solvent_components=[SolventComponentSpec(name="methanol", volume_fraction=0.5)],
        )
...
def test_cinnamaldehyde_default_count_is_deterministic() -> None:
    ...
    expected = round_half_up(0.05 * 125.0 * NM3_TO_L * AVOGADRO_CONSTANT_MOL_INV)

    assert plan.reactants[0].name == "cinnamaldehyde"
    assert plan.reactants[0].count == expected == 4
...
def test_volume_fractions_must_not_exceed_one() -> None:
    """Reject schema-bypassed solvent volume fractions above one."""

    with pytest.raises(ValueError, match=r"volume fractions must not exceed 1\.0"):
        plan_solution_components(
            box_volume_nm3=125.0,
            solvent_components=[
